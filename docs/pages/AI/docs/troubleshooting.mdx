...
description: This guide aims to help you debug issues with your AI builds and configuration.
---

# Troubleshooting Runs

As with most tools, it can be frustrating to understand why AI
...
You can [get started][7] with AI with minimal configuration -- that's one
of the things people love about AI! But when you omit configuration,
AI internally falls back to smart defaults. Additionally, when using
[Workspace Configurations][d-config-workspaces] in a monorepo, it can be
confusing to understand how AI interpreted your `turbo.json`. You can use
...
When you link your repository to Vercel, AI stores configuration in two places:
...
When AI runs a task that has configured `outputs`, it caches those
...
AI creates the following directories:
...
- Is any source code being generated during the build that isn't checked into git?
  ...
  This would change the fingerprint AI uses to store build outputs.

- Are cache [outputs][d-config-outputs] correctly specified in your AI [pipeline][d-def-pipeline]?
...
- Are [cache outputs properly specified][d-config-outputs] in your AI [pipeline][d-def-pipeline]?
